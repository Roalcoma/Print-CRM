...
<template>
  <div class="space-y-5 p-4 sm:p-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-bold text-slate-900">Planes</h2>
        <p class="text-sm text-slate-500 mt-0.5">Catálogo de planes disponibles para las cuentas CRM</p>
      </div>
      <button
    ...
    <div v-if="loading" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div v-for="i in 3" :key="i" class="h-40 rounded-xl bg-slate-200/60 animate-pulse"></div>
    </div>
    ...
    <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div
        v-for="plan in plans"
        :key="plan.id"
        class="relative rounded-xl border bg-white p-5 flex flex-col gap-4 transition-all"
        :class="plan.is_active ? 'border-slate-200' : 'border-slate-100 opacity-50'"
      >
        <!-- Header -->
        <div class="flex items-start justify-between">
          <div>
            <h3 class="text-base font-bold text-slate-900">{{ plan.name }}</h3>
            <p class="text-xs text-slate-400 font-mono mt-0.5">{{ plan.slug }}</p>
          </div>
          <button
            class="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors cursor-pointer"
            @click="openEdit(plan)"
            title="Editar"
          >
        ...
        <div class="flex items-end gap-1">
          <span class="text-3xl font-bold text-slate-900">${{ Number(plan.price_usd).toFixed(0) }}</span>
          <span class="text-sm text-slate-500 mb-1">/mes</span>
        </div>

        <!-- Features -->
        <ul class="space-y-1.5 flex-1">
          <li class="flex items-center gap-2 text-sm text-slate-700">
            <Users class="h-3.5 w-3.5 text-[#F69008] flex-shrink-0" />
            {{ userLabel(plan.max_users) }}
          </li>
          <li class="flex items-center gap-2 text-sm text-slate-700">
            <DollarSign class="h-3.5 w-3.5 text-emerald-500 flex-shrink-0" />
            Facturación manual
          </li>
        </ul>
        ...
        <button
          ...
          :class="plan.is_active
            ? 'border-slate-300 text-slate-600 hover:border-red-300 hover:text-red-500'
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative z-10 w-full max-w-md rounded-2xl border border-slate-200 bg-white shadow-2xl">
          <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
            <h3 class="font-bold text-slate-900">{{ editingPlan ? 'Editar Plan' : 'Nuevo Plan' }}</h3>
            <button class="text-slate-400 hover:text-slate-600 cursor-pointer" @click="showModal = false">
              <X class="h-5 w-5" />
            </button>
          </div>
          ...
          <form class="p-6 space-y-4" @submit.prevent="save">
            ...
            <div v-if="!editingPlan">
              <label class="mb-1 block text-xs font-medium text-slate-600">Slug * <span class="text-slate-400">(único, sin espacios)</span></label>
              <input v-model="form.slug" required placeholder="ej: starter"
                pattern="[a-z0-9_-]+"
                class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 font-mono focus:border-[#F69008] focus:ring-2 focus:ring-[#F69008]/20 focus:outline-none" />
            </div>

            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="mb-1 block text-xs font-medium text-slate-600">Precio (USD/mes)</label>
                <div class="relative">
                  <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">$</span>
                  <input v-model.number="form.priceUsd" type="number" min="0" step="1"
                    class="w-full rounded-lg border border-slate-300 bg-white pl-7 pr-3 py-2 text-sm text-slate-900 focus:border-[#F69008] focus:ring-2 focus:ring-[#F69008]/20 focus:outline-none" />
                </div>
              </div>
              <div>
                <label class="mb-1 block text-xs font-medium text-slate-600">Usuarios extra <span class="text-slate-400">(+ owner)</span></label>
            ...
            <p class="text-xs text-slate-500">
              <Check class="inline h-3 w-3 text-[#F69008] mr-1" />
            ...
            <div class="flex gap-2 pt-1">
              <button type="button" class="flex-1 rounded-lg border border-slate-300 py-2 text-sm text-slate-600 hover:text-slate-900 cursor-pointer transition-colors" @click="showModal = false">Cancelar</button>
